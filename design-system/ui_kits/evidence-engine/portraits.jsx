// Noir suspect busts — split-lit, drawn not photographed. Exported to window
// for the UI kit app. Mirrors the design system's SuspectPortrait component.
function EEPortrait({ who, pressure = 0 }) {
  const shadow =
    pressure > 0 ? `rgba(126, 29, 31, ${0.35 + pressure * 0.45})` : "#0e0b08";
  const uid = who;
  return (
    <svg viewBox="0 0 120 130" className="kit-portrait__svg" role="img" aria-label="Suspect portrait">
      <defs>
        <linearGradient id={`klamp-${uid}`} x1="0" y1="0" x2="1" y2="0.2">
          <stop offset="0%" stopColor="#e9d8ae" />
          <stop offset="55%" stopColor="#caa86a" />
          <stop offset="100%" stopColor="#7d6238" />
        </linearGradient>
        <linearGradient id={`kcoat-${uid}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="#3b3122" />
          <stop offset="100%" stopColor="#171209" />
        </linearGradient>
      </defs>
      <circle cx="60" cy="56" r="44" fill={`url(#klamp-${uid})`} opacity="0.14" />
      {who === "curator" && (
        <g>
          <path d="M18 130 C22 102 40 94 60 94 C80 94 98 102 102 130 Z" fill={`url(#kcoat-${uid})`} />
          <path d="M52 96 L60 110 L68 96 L60 92 Z" fill="#0d0a07" />
          <rect x="53" y="74" width="14" height="22" rx="6" fill={`url(#klamp-${uid})`} />
          <path d="M40 44 C40 28 49 20 60 20 C71 20 80 28 80 44 C80 62 72 74 60 74 C48 74 40 62 40 44 Z" fill={`url(#klamp-${uid})`} />
      ...
      {who === "assessor" && (
        <g>
          <path d="M22 130 C26 104 42 96 60 96 C78 96 94 104 98 130 Z" fill={`url(#kcoat-${uid})`} />
          <path d="M52 98 L60 112 L68 98 L60 94 Z" fill="#d8c9a4" />
          <path d="M58 98 L60 110 L62 98 L60 95 Z" fill="#5a2020" />
          <rect x="53" y="76" width="14" height="22" rx="6" fill={`url(#klamp-${uid})`} />
          <path d="M42 46 C42 30 50 22 60 22 C70 22 78 30 78 46 C78 64 70 78 60 78 C50 78 42 64 42 46 Z" fill={`url(#klamp-${uid})`} />
      ...
      {who === "collector" && (
        <g>
          ...
          <rect x="53" y="74" width="14" height="22" rx="6" fill={`url(#klamp-${uid})`} />
          <path d="M41 46 C41 30 49 22 60 22 C71 22 79 30 79 46 C79 62 71 75 60 75 C49 75 41 62 41 46 Z" fill={`url(#klamp-${uid})`} />
