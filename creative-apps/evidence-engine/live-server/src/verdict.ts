/**
 * Evidence-relative verdict heuristics, evolved from the MCP server's
 * check_claim (cycle-13 fix): temporal conflicts and explicit negation
 * phrases only — and only inside segments (sentences/log lines) that share
 * a meaningful term with the claim, so a statement-header timestamp or a
 * stray "was not unusual" elsewhere in a document cannot manufacture a
 * contradiction. All verdicts are relative to the case file or to the
 * speaker's own indexed testimony, never absolute truth.
 */
