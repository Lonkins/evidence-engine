...
| **A4** | ✅ **Full `activity[]` stream in the wiretap** (June 13) | Best Use of IQ (engineer) | S | DONE. `kbReason` parses the KB's native `activity[]` into engine-tap lines: `IQ ▸ plan retrieval`, `IQ ▸ search the source` (often 5× — genuine multi-step agentic retrieval), `IQ ▸ synthesise verdict`, `IQ ▸ agentic reasoning`. The judge sees Foundry IQ thinking, not just latency. Verified via API on a challenge. |
